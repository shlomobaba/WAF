...
import pickle
def createDataSet():
    dataset=pandas.read_csv("XSS_dataset.csv")
    temp_sentences=list(dataset["Sentence"])
    sentences=[]
    labels=list(dataset["Label"])
    for sentence in temp_sentences:
        texts=(re.compile(r"""([<]|[>])""")).split(sentence)
        texts=" ".join(texts)
...
def main():
    ...
    xss_model.add(keras.layers.Embedding(input_dim=10000, output_dim=128))
